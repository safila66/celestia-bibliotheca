...
<div class="page-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; flex-wrap: wrap; gap: 15px;">
    <h2 style="font-family: 'Cinzel', serif; color: var(--gold); font-weight: bold; margin: 0; font-size: 24px;">Book Delivery</h2>
    
    <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
        <a href="javascript:void(0);" onclick="createDelivery()" style="padding: 8px 16px; background: var(--gold); color: #04060f; border: none; font-weight: bold; text-decoration: none; border-radius: 4px;">+ Tambah Pengiriman</a>
    </div>
</div>
        ...
        <table style="width: 100%; border-collapse: collapse; text-align: left; color: var(--ivory); min-width: 900px;">
            <thead>
                <tr style="border-bottom: 2px solid var(--gold); background: rgba(201,168,76,0.1);">
                    ...
                    <th style="padding: 12px 10px; font-family: 'Raleway', sans-serif; font-size: 11px; color: var(--gold); text-transform: uppercase;">Buku</th>
                    <th style="padding: 12px 10px; font-family: 'Raleway', sans-serif; font-size: 11px; color: var(--gold); text-transform: uppercase;">Tgl Pengiriman</th>
                    <th style="padding: 12px 10px; font-family: 'Raleway', sans-serif; font-size: 11px; color: var(--gold); text-transform: uppercase;">Resi</th>
            ...
            <tbody>
                <?php if (!empty($deliveries)): ?>
                    <?php $i = 1; foreach ($deliveries as $row): ?>
                    <tr style="border-bottom: 1px solid rgba(201,168,76,0.2); transition: background 0.3s;" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='transparent'">
                        ...
                        <td style="padding: 12px 10px; color: var(--text-dim); font-size: 13px;">
                            <?= !empty($row['shipping_date']) ? date('d/m/Y', strtotime($row['shipping_date'])) : '-' ?>
                        </td>
                        <td style="padding: 12px 10px; color: var(--moon-silver); font-size: 13px;"><?= esc($row['tracking_number'] ?? '-') ?></td>
                        <td style="padding: 12px 10px;">
                            <?php
                                $statusMap = [
                                    'pending'   => ['label' => 'Menunggu',  'bg' => 'rgba(212, 184, 114, 0.15)', 'fg' => '#d4b872', 'border' => '#d4b872'],
                                    'shipping'  => ['label' => 'Dikirim',   'bg' => 'rgba(126, 200, 160, 0.15)', 'fg' => '#7ec8a0', 'border' => '#7ec8a0'],
                                    'delivered' => ['label' => 'Selesai',   'bg' => 'rgba(150, 150, 150, 0.15)', 'fg' => '#a0a0a0', 'border' => '#707070'],
                                    'cancelled' => ['label' => 'Dibatalkan','bg' => 'rgba(224, 112, 112, 0.15)', 'fg' => '#e07070', 'border' => '#e07070'],
                                ];
                        ...
                        <td style="padding: 12px 10px; text-align: center;">
                            <div style="display: flex; gap: 6px; justify-content: center;">
                                <button onclick="editDelivery(<?= $row['id'] ?>)" style="background: #c9a84c; color: #04060f; border: none; padding: 6px 10px; border-radius: 4px; font-size: 11px; cursor: pointer; font-weight: bold;" title="Edit">✏️</button>
                                <a href="<?= base_url('admin/delete/delivery/' . $row['id']) ?>" style="background: #963c3c; color: white; padding: 6px 10px; border-radius: 4px; text-decoration: none; font-size: 11px; font-weight: bold;" onclick="return confirm('Apakah Anda yakin ingin menghapus catatan ini?')" title="Hapus">🗑️</a>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
...
<script>
    function editDelivery(id) {
        ...
        $.ajax({
            url: "<?= base_url('admin/ajax/edit/delivery/') ?>" + id,
            type: "GET",
    ...
    function createDelivery() {
        ...
        $.ajax({
            url: "<?= base_url('admin/ajax/create/delivery') ?>",
            type: "GET",
            success: function(response) {
