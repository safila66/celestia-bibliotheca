...
<div class="page-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
    <h2 style="font-family: 'Cinzel', serif; color: #e07070; font-weight: 400; letter-spacing: 0.05em;">Tong Sampah Admin</h2>
    <div style="display: flex; gap: 10px;">
        <a href="<?= base_url('admin/list/superadmin') ?>" class="btn-ghost" style="padding: 8px 16px; text-decoration: none; border: 1px solid var(--gold); color: var(--gold);">⬅ Kembali ke Manajemen Admin</a>
    </div>
</div>
                ...
                <?php if (!empty($admins)): ?>
                    ...
                    <tr style="border-bottom: 1px solid rgba(224,112,112,0.1); transition: background 0.3s;" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='transparent'">
                        ...
                        <td style="padding: 16px;">
                            <div style="font-family: 'Cinzel', serif; font-size: 15px; font-weight: 600; color: var(--moon-silver); text-decoration: line-through;"><?= esc($admin['name']) ?></div>
                        </td>
                        ...
                        <td style="padding: 16px; color: #e07070; font-size: 13px;">
                            <?= date('d M Y, H:i', strtotime($admin['deleted_at'])) ?>
                        </td>

                        <td style="padding: 16px; text-align: center;">
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                <a href="<?= base_url('admin/restore/superadmin/' . $admin['id']) ?>" style="background: #7ec8a0; color: #04060f; padding: 6px 12px; border-radius: 4px; text-decoration: none; font-size: 11px; font-weight: bold;">✨ Restore</a>
                                
                                <a href="<?= base_url('admin/purge/superadmin/' . $admin['id']) ?>" style="background: transparent; border: 1px solid #e07070; color: #e07070; padding: 6px 12px; border-radius: 4px; text-decoration: none; font-size: 11px; font-weight: bold;" onclick="return confirm('Peringatan: Tindakan ini tidak dapat dibatalkan. Hapus admin ini selamanya?')">🔥 Hapus Permanen</a>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="5" style="text-align: center; padding: 40px; color: var(--text-dim); font-style: italic;">
                            ...
                            Tidak ada admin di tong sampah.
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
